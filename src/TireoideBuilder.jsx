...
export default function TireoideBuilder({ mascaraTexto, aoAtualizar }) {
  ...
  useEffect(() => {
    aoAtualizar(montarLaudoTireoide(mascaraTexto, gland, nodules, nodulosPadrao));
  }, [mascaraTexto, gland, nodules, nodulosPadrao, aoAtualizar]);
  ...
  const adicionarNoduloPadrao = (lobo = "") => setNodulosPadrao((prev) => [...prev, { ...NODULO_PADRAO_VAZIO, lobo }]);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-[var(--c-slate-800)] rounded-lg border border-[var(--c-slate-700)] p-3 space-y-3">
        <div className="text-xs font-semibold text-[var(--c-slate-400)] uppercase tracking-wide">Glândula</div>
        ...
        <div className="text-sm text-[var(--c-slate-400)]">
          Volume: lobo direito <b className="text-[var(--c-slate-200)]">{fmtVol(rdVol)}</b> cm³ · lobo esquerdo{" "}
          <b className="text-[var(--c-slate-200)]">{fmtVol(leVol)}</b> cm³ · total{" "}
          <b className="text-[var(--c-slate-200)]" data-testid="volume-total">{fmtVol(totalVol)}</b> cm³
        </div>
      </div>

      {/* Quantidade de nódulos */}
      <div className="bg-[var(--c-slate-800)] rounded-lg border border-[var(--c-slate-700)] p-3">
        <div className="text-xs font-semibold text-[var(--c-slate-400)] uppercase tracking-wide mb-2">Nódulos (ACR TI-RADS)</div>
      ...
      {/* Nódulo padrão: atalho para o caso banal, com tudo pré-selecionado
          exceto o que varia por paciente. Convive com os nódulos detalhados
          acima (caminho completo, para o caso não banal) — nenhum dos dois
          é removido ou alterado pelo outro. */}
      <div className="bg-[var(--c-slate-800)] rounded-lg border border-[var(--c-slate-700)] p-3 space-y-3">
        <div className="flex items-center justify-between">
          <div className="text-xs font-semibold text-[var(--c-slate-400)] uppercase tracking-wide">Nódulo padrão</div>
          <div className="flex items-center gap-1">
            <button onClick={() => adicionarNoduloPadrao()} className="px-2.5 py-1 rounded-full text-xs border border-[var(--c-accent-600)] text-[var(--c-accent-300)] hover:bg-[var(--c-accent-950)]">
              + Nódulo padrão
            </button>
            <button onClick={() => adicionarNoduloPadrao("direito")} title="Nódulo padrão — lobo direito, já selecionado" className="px-2 py-1 rounded-full text-xs border border-[var(--c-accent-600)] text-[var(--c-accent-300)] hover:bg-[var(--c-accent-950)]">
              D
            </button>
            <button onClick={() => adicionarNoduloPadrao("esquerdo")} title="Nódulo padrão — lobo esquerdo, já selecionado" className="px-2 py-1 rounded-full text-xs border border-[var(--c-accent-600)] text-[var(--c-accent-300)] hover:bg-[var(--c-accent-950)]">
              E
            </button>
          </div>
        </div>
        {nodulosPadrao.map((n, i) => (
          <div key={i} className="border border-[var(--c-slate-700)] rounded-md p-3 space-y-2" data-testid={`nodulo-padrao-${i + 1}`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              <div className="sm:col-span-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="px-2 py-0.5 rounded-full text-[11px] font-mono font-bold bg-[var(--c-accent-500)] text-[var(--c-on-accent)]" data-testid={`tirads-padrao-${i + 1}`}>
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <input value={n.m3} onChange={(e) => setNodPadraoField(i, "m3", e.target.value)} placeholder="1,1" inputMode="decimal" className={inputCls} />
              </div>
            </div>
          </div>
        ))}
      ...
      {nodules.map((n, i) => {
        const s = scoreFor(n);
        return (
          <div key={i} className="bg-[var(--c-slate-800)] rounded-lg border border-[var(--c-slate-700)] p-3 space-y-2" data-testid={`nodulo-${i + 1}`}>
            <div className="flex items-center gap-2">
              <div className="text-sm font-semibold flex-1">N{i + 1}</div>
              <span className="px-2 py-0.5 rounded-full text-[11px] font-mono bg-[var(--c-slate-700)] text-[var(--c-slate-200)]" data-testid={`pts-${i + 1}`}>
                {s.pts} pts
              </span>
              <span className="px-2 py-0.5 rounded-full text-[11px] font-mono font-bold bg-[var(--c-accent-500)] text-[var(--c-on-accent)]" data-testid={`tr-${i + 1}`}>
                {s.tr}
              </span>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            ...
            <div className="text-xs text-[var(--c-slate-400)] leading-relaxed">
              <span className="text-[var(--c-slate-300)] font-semibold">{s.sub}</span> · Risco de malignidade: {s.risk}
              <br />
              PAAF: <span className="text-[var(--c-slate-200)]">{s.fna}</span> · Seguimento: <span className="text-[var(--c-slate-200)]">{s.follow}</span>
            </div>
          </div>
        );
      })}
